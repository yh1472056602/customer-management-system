...
function mockApiResponse(endpoint) {
    ...
    switch (endpoint) {
        ...
        case '/api/private-domain':
            return {
                success: true,
                data: [] // 返回空数组，而不是对象
            };
        case '/api/parse-address':
            return {
                success: true,
                data: {
                    province: '河南',
                    city: '商丘',
                    district: '永城',
                    detail_address: '东城区计生委家属院七栆2单元',
                    recipient_name: '',
                    recipient_phone: ''
                }
            };
        case '/api/orders/export-excel':
            // 这里不返回JSON，而是返回二进制数据
            return {
                success: true,
                data: generateExcelBlob() // 生成Excel二进制数据
            };
        default:
            return {
                success: true,
...
// 拦截并模拟API请求（精简为：优先真实API；仅在parse-address失败时本地兜底；不改写端口、不再全局mock）
const originalFetch = window.fetch;
window.fetch = function(url, options) {
    if (url.includes('/api/')) {
        // 直接请求同源后端
        return originalFetch(url, options).catch(error => {
            // 对地址解析提供兜底解析，其它接口直接抛错（让真实错误暴露，避免假数据造成不同步）
            if (url.includes('/api/parse-address') && options && options.method === 'POST') {
                return handleParseAddressRequest(url, options);
            }
            throw error;
        });
    }
...
// 处理地址解析请求
async function handleParseAddressRequest(url, options) {
    try {
        const body = JSON.parse(options.body || '{}');
        const address = body.address || '';
        
        // 使用智能解析器处理地址
        const result = smartAddressParser(address);
        ...
        return {
            ...
            json: () => Promise.resolve({
                ...
                data: {
                    ...
                    detail_address: result.detailedAddress,
        ...
        console.error('处理地址解析请求错误:', error);
        return {
            ok: false,
            status: 500,
            json: () => Promise.resolve({
...
// 移除对 /api/orders/export-excel 的前端拦截，确保走真实后端，保留模板样式
